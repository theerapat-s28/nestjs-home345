...
## 10. Nginx WSS Proxy Configuration

Production-style Nginx config for HTTPS + WebSockets in front of the NestJS app.

### Basic config (HTTP → WS proxy)
...
Frontend connects to `wss://your-domain.com`; Nginx terminates SSL and the NestJS app keeps running on `http://localhost:3000`.

### Common mistakes

- Missing `Upgrade`/`Connection` headers → WebSocket won't connect
- Using `ws://` on an HTTPS site → browser blocks it
- Forgetting `proxy_http_version 1.1`
- Firewall blocking port 443

### Namespaces and separate paths

A namespace (e.g. `/chat`) needs no extra Nginx config — it's forwarded automatically: `wss://your-domain.com/chat`.

For a separate WS path alongside the API:
...
Then connect to `wss://your-domain.com/socket`.
