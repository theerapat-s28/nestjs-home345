...
export class PortfolioCashController {
  ...
  @Get()
  @ApiOperation({
    summary: "Get cash balances for all currencies in the portfolio",
  })
  ...
  @ApiOperation({
    summary: "Record a manual cash operation (DEPOSIT / WITHDRAWAL / etc.)",
  })
  @ApiParam({ name: "portfolioId", description: "Portfolio UUID" })
  recordOperation(
    @Request() req: any,
    ...
    return this.portfolioCashService.recordOperation(
      portfolioId,
      req.user.id,
      dto,
    );
  ...
  getCashTransactions(
    ...
    return this.portfolioCashService.getCashTransactions(
      portfolioId,
      req.user.id,
      query,
    );
